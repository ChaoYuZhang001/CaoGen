...
- [ ] M2-T2 onboarding 打磨:首个任务模板、Provider 配置向导、失败时的引导文案
  ...
  - [x] 数字员工计费可信增量：每个终态 ModelAttempt 现在把 Session/Run、冻结的 worker/Assignment、Provider/model、usage 和可审计 USD 成本写入 fsync 的哈希链 JSONL；OpenAI 使用配置或内建定价，Anthropic/Gemini 只使用明确的 Provider 模型定价。预算策略优先读取该账本，篡改、未计价成功请求或账本读取失败均 fail-closed；无新账本记录时才兼容旧会话成本。专项账本、数字员工策略与 Provider dispatch required gate 均通过。全局月预算仍需迁移到同一账本，真实付费账号跨月对账仍开放。
