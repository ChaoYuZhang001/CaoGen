...
- [ ] M1-T6 发布 v0.1.7,Release Notes 只写当前真实能力,不写路线图功能
  ...
  - [ ] 发布自动化源码变化使 run `30150795350` 只保留为历史证据；`8ae1e207` 本地 Deep 已 `159 total / 157 required pass / 2 optional skip / 0 blocked / 0 fail` 且 git clean/unchanged。`d5f0a86f` 的 run `30157730258` 已通过身份/源码门禁但遭遇 GitHub macOS runner internal error，后续 run `30157949745` / `30158101444` 均为 0-job 平台失败。`bd26eaa3` 的 run `30159248949` 已通过身份/源码、Intel 依赖/凭据和 release-scope P2，但 exact-commit Deep 暴露 ModelAttempt crash E2E worker 只等待 pending Promise、可能在父进程 `SIGKILL` 前退出的测试夹具竞态，签名/公证/打包按门禁跳过；keepalive 修复本地 targeted 连续 `20/20` 通过。把修复绑定到新 exact commit，重跑 Intel-only 候选并完成预发布交接检查
