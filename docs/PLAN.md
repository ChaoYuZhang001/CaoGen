...
- [ ] `IDE-001`：项目树、搜索、多标签、LSP、诊断、Diff、终端、Git、Preview、测试、断点调试、跨文件重构和崩溃恢复均已在 CaoGen 内建；仍需代表性 50,000 文件混合语言仓库、网络文件系统/持续 watcher storm 和真人重度任务黄金门禁后才能关闭。
- [ ] `IDE-002`：断点调试 `16/16 + 13/13`、崩溃恢复 `36/36 + 12/12`、5,201 文件索引基线和工作台键盘可达 `8/8` 已通过；仍需代表性 50,000 文件混合语言、网络文件系统/持续 watcher storm 和真人重度任务黄金门禁；旧 IDE 插件不得作为证据。
...
- [ ] `IDE-001`、`IDE-002` 仍保持开放：崩溃 journal/恢复和工作台标签键盘覆盖已补齐，尚缺代表性 50,000 文件混合语言仓库、网络文件系统/持续 watcher storm 和真人重度任务黄金门禁；当前能力仍不是 1.0 发布验收。
