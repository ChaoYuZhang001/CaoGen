...
- [x] M1-T6 发布 v0.1.7,Release Notes 只写当前真实能力,不写路线图功能
  ...
  - [ ] 等待期功能修复收口后重跑 v0.1.8 clean Intel 候选，再把新候选的精确五资产和四类报告写入 Final Notes，并在 clean publication-only 后继提交上通过 scoped notes/handoff preflight；`9591c20b / 30215660873` 已被后续固定模型/Drive 修复超越，只保留为历史签名证据。获得创始人针对 v0.1.8 的新授权后才可创建 tag/Release
- [ ] M1 退出验收:首位陌生用户从官网下载、无安全绕过安装并按 Quick Start 完成第一个只读任务
  ...
  - [x] 等待测试者期间修复固定模型切换 Drive 被错误清空：有效 Provider/模型继续保留，只在模型真实失效时 fail-closed；新 Drive 的默认权限仍同步更新。真实 Electron `2026-07-26T20-07-47-489Z` 为 `12/12`、9 张截图，页面操作 `2026-07-26T20-08-40-109Z` 为 `22/22`，build/typecheck 通过；当前仍需 clean Deep 和后继 Intel 候选，不能替代真人 M1 或 v0.1.7 Release 完整性恢复
  - [x] 等待测试者期间修复运行中重复发送导致的草稿丢失与假排队：Composer 运行中可继续起草但发送被禁用，Browser 批注和产物 Preview 也遵守同一忙碌门禁；主进程未确认接受前不清空文本/附件，拒绝时撤回乐观消息、恢复原状态，IPC/IDE Bridge 如实返回失败，记忆建议与自动提取不再由被拒消息触发。真实 Electron live-switch `2026-07-27T03-28-37-915Z` 为 `6/6`，10 次 Assistant/Studio 往返后草稿仍在、运行中 Enter 不发送、模型请求保持 1 个、底层重复发送返回 `false`，结束后恢复发送；页面操作 `2026-07-27T03-26-54-546Z` 为 `22/22`，连续 Browser/Preview 发送等待真实可发送状态。build/typecheck、IDE Bridge、模型切换策略、Effect close-race 和编码标准 required gate 通过。该 targeted 证据仍需提交后的 clean Deep 与后继 Intel 候选，不替代真人 M1 或 v0.1.7 Release 完整性恢复
