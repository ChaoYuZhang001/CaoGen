...
当前基线(2026-07-22,以 STATUS.md 为准):64 个 P0 中 21 个已验证。
官网"5 完成/17 部分/42 立项"口径需在下次发布前与 STATUS 对齐(见 M1-T4)。
...
- [ ] M1-T1 提交并整理本地未提交的全部工作(约一周量),git 历史恢复连续
- [ ] M1-T2 版本号定为 0.1.7(package.json 从 1.0.0 回退,延续 0.1.x 发布线);1.0 版本号预留给完整 Work OS
- [ ] M1-T3 macOS(x64 + arm64)签名 + 公证,Windows x64 签名;发布管线走通
- [ ] M1-T4 三处口径对齐:README、官网、STATUS.md 的完成度数字与措辞一致
- [ ] M1-T5 文档收敛:旧规划文档移入 docs/archive/,README 增加对比表与可见 Roadmap
