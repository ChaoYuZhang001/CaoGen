...
- [ ] `IDE-001`：项目树、搜索、多标签、LSP、诊断、Diff、终端、Git、Preview、测试、断点调试、跨文件重构和崩溃恢复均已在 CaoGen 内建；代表性 50,000 文件混合语言门禁和 Windows loopback SMB/UNC 持续 watcher storm 已通过，仍需远程网络断连/抖动和真人重度任务黄金门禁后才能关闭。
- [ ] `IDE-002`：断点调试 `16/16 + 13/13`、崩溃恢复 `36/36 + 12/12`、工作台键盘可达 `8/8` 已通过；代表性 50,000 文件混合语言门禁以冷索引 `29.3s`、warm reopen `2.52s` 通过，并证明第 50,001 个文件被明确截断；Windows loopback SMB/UNC 门禁完成 16 波 1,392 次操作、精确收敛和重启读回，仍需远程网络断连/抖动和真人重度任务黄金门禁；旧 IDE 插件不得作为证据。
...
- [ ] `IDE-001`、`IDE-002` 仍保持开放：崩溃 journal/恢复、工作台标签键盘、代表性 50,000 文件混合语言索引及 Windows loopback SMB/UNC 持续 watcher storm 门禁已补齐，尚缺远程网络断连/抖动和真人重度任务黄金门禁；当前能力仍不是 1.0 发布验收。
