...
| TRUST-002 | P0 | 部分完成（versioned entry inventory foundation） | 版本化入口清单与 AST required gate 已覆盖 156 个 Electron IPC、52 个 Agent 工具和 79 个 ProjectWorkspace/DigitalWorker/Supervisor 嵌套动作；新增、遗漏或重复入口会因 inventory drift 失败，自动化 Agent 外部工具必须归入 queryable、conditional 或 opaque，未知工具保持 opaque 并禁止自动重放。25 个 direct-user 外部 IPC 已显式登记为 `replay: never`，但仍未进入 Effect Ledger；Issue、消息、可查询 MCP 与其余直达入口的专用对账继续开放。 |
