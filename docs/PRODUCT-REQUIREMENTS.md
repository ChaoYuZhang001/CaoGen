...
| TRUST-002 | P0 | 部分完成（versioned entry inventory foundation） | inventory v3 与 AST required gate 已覆盖 156 个 Electron IPC、52 个 Agent 工具和 79 个 ProjectWorkspace/DigitalWorker/Supervisor 嵌套动作；新增、遗漏或重复入口会因 inventory drift 失败，自动化 Agent 外部工具必须归入 queryable、conditional 或 opaque，未知工具保持 opaque 并禁止自动重放。`attachments:copyImage` 与 `attachments:saveImageBytes` 已进入不持久化原始图片或源路径的 opaque `attachment_write` Effect，并以 `waiting_reconciliation` 处理未知结果；`projectContext:write` 已进入 queryable `file_content` Effect，正文不持久化到任务数据库，强杀后只读对账且不自动重放。其余 22 个 direct-user 外部 IPC 仍只登记为 `replay: never` 且未进入 Effect Ledger，Issue、消息、可查询 MCP 与其余直达入口的专用对账继续开放。 |
