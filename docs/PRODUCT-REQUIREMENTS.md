...
| TRUST-002 | P0 | 部分完成（versioned entry inventory foundation） | inventory v6 与 AST required gate 已覆盖 156 个 Electron IPC、52 个 Agent 工具和 79 个 ProjectWorkspace/DigitalWorker/Supervisor 嵌套动作；新增、遗漏或重复入口会因 inventory drift 失败，自动化 Agent 外部工具必须归入 queryable、conditional 或 opaque，未知工具保持 opaque 并禁止自动重放。`attachments:copyImage` 与 `attachments:saveImageBytes` 已进入不持久化原始图片或源路径的 opaque `attachment_write` Effect，并以 `waiting_reconciliation` 处理未知结果；`projectContext:write` 已进入 queryable `file_content` Effect，正文不持久化到任务数据库，强杀后只读对账且不自动重放；`plugins:probeMcp` 已进入 opaque `mcp_probe` Effect，durable target 仅含条目/配置摘要和 transport，不保存 URL、命令、参数、环境变量、Header、凭据或服务错误正文，SIGKILL 后等待人工对账且不重放探测。`plugins:installLocal` 与 `plugins:uninstall` 已进入 queryable managed-plugin Effect：冻结插件名、根/锚点身份、内容摘要、文件/字节计数和同根 staging/trash 路径，不持久化源路径、源内容或原始错误；原子切换/回收支持完整强杀后的只读确认，部分 checkpoint 保持 `waiting_reconciliation` 且不重放。`terminals:start/write/resize/close` 已进入 opaque `terminal_action` Effect：Terminal/Session/CWD 只保存摘要，write 仅保存输入 SHA-256 与字节数而不保存命令文本，未知结果等待人工对账并禁止自动重放。其余 15 个 direct-user 外部 IPC 仍只登记为 `replay: never` 且未进入 Effect Ledger，Issue、消息、可查询 MCP 与其余直达入口的专用对账继续开放。 |
