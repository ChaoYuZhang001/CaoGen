...
2026-08-10 CC Switch 差距校正：CaoGen 已实现 Provider 批量迁移、MCP/Prompt/Skill 资产迁移、历史用量迁移和 Provider 可靠性策略迁移，不再只依赖通用 Profile JSON。Provider 链路覆盖 Codex/Claude、OpenAI Responses/Chat 和 Anthropic Messages，迁移模型、定价、成本倍率、月预算、端点与凭据，并从 `proxy_config` 按来源 App 独立迁入自动切换、最大恢复次数、五项熔断参数和三类超时；运行时按 Provider 覆盖全局设置。首字节计时覆盖 fetch 到首个非空响应块，随后按响应块重置流空闲计时，非流式总超时覆盖网络与响应消费；超时进入 Provider 网络故障、熔断和安全 failover，用户取消保持独立语义。代理监听/接管/日志明确忽略。资产链路将安全 MCP 字段写入受管配置，将 Prompt 转为 CaoGen prompt-only Skill，并复制 CC Switch Skill，敏感内容在 Renderer 前阻断；历史用量链路读取 45 条 `usage_daily_rollups`，不复制 21,780 条逐请求日志或伪造 CaoGen ModelAttempt，将请求/成功数、四类 Token、成本和加权延迟合并到 Usage 总计、趋势与 Provider/模型统计，原生最近请求和凭据归属保持隔离。链路共享来源/目标重验、私有批次备份和回滚边界。Provider 批次 apply/rollback 已绑定同一 operation journal，启动和读取时只按精确 before/desired Store digest 对账、不自动重放；真实跨进程 `SIGKILL` gate `12/12` 覆盖 prepare 后、apply commit 后和 rollback commit 后强杀，凭据保留、备份无明文且来源数据库字节不变。当前其余证据为 Provider `37/37 + 13/13`、超时 `15/15`、熔断 `11/11`、资产 `22/22 + 18/18`、Usage `43/43 + 25/25 + 35/35`；本机真实库只读预览确认 Claude/Codex 独立策略，资产预览识别 MCP、Prompt、Usage 三项且均可迁移，数据库前后 SHA 不变。当前仍不能声称替代 CC Switch：云同步和真实库 apply 尚未完成。

2026-08-10 Provider 同步进度：已实现凭据隔离的本地同步目录，可直接使用 OneDrive、Dropbox、iCloud 或 NAS。同步包络记录父版本和内容摘要，保留远端历史，区分本机领先、远端领先与双方分叉；发布前复核本机/远端摘要，远端应用沿用逐项决策、私密备份和 Provider operation journal。原生 WebDAV 已覆盖 `PROPFIND / MKCOL / GET / PUT`、Basic Auth、不可变历史和 ETag CAS；原生 S3/S3-compatible 已覆盖 AWS SigV4、Region/Bucket/Prefix、自定义 HTTPS 端点、path-style、临时 Session Token、不可变历史和 ETag 条件写入。两类云凭据均只保存在系统加密的 Provider 凭据记录中，公网 HTTP 在联网前拒绝。安全自动发布只处理远端缺失或本机领先；显式启用自动拉取后只应用曾同步祖先上的远端单方新版本，首次连接、歧义项和分叉均停下等待人工。设置页可列出最近 20 个不可变远端版本，逐项预览并经私密备份和 operation journal 恢复，历史浏览不改写 current。专项 smoke 为本地目录 `26/26`、WebDAV `35/35`、S3 `36/36`，Provider Profile `145/145`、跨进程恢复 `13/13`、Electron UI `70/70` 均通过。本机真实 CC Switch Provider 在隔离 CaoGen 目录完成 apply/rollback `16/16`，真实资产副本在隔离 home 完成 apply/rollback `12/12`，源数据库均字节不变，技术项 `MIG-001` 已关闭。当前仍不能声称完整替代 CC Switch：真人正式 UI 迁移计时、回退原工具次数和留存证据尚未完成。
...
2. Docker 产品模式已删除，历史 strict 设置会进入 fail-closed 确认态；Claude SDK 自动放行、原生设置 hooks/statusLine、Git filter 和 ripgrep config 旁路已封。新 Provider Key 的可逆 Base64 fallback 也已移除；GUI 五分钟授权已从持久全局开关收紧为主进程内存中的 session/cwd/action/精确输入 grant，权限设置页已切换到版本化结构化规则并兼容迁移旧文本字段。持久规则可按 AND 组合匹配工具、路径、命令、网络 hostname、GUI process/window、普通或动态 MCP tool、MCP 非敏感标量参数、有效 postcondition 和风险条件；参数范围用 RFC 6901 pointer 与 wildcard 值模式成对表达，敏感字段和结构化值 fail-closed。动态 `mcp__server__tool` 保持 high risk，工具名或参数漂移不命中规则，Full Access 不能静默放行。非 GUI 稳定文件/Office/bash/Git 操作也已支持绑定 session/cwd/tool/完整 input digest 与 frozen Effect target/diff digest 的五分钟 grant，审批卡显示输入与 Effect scope。high/critical 与 MCP/push/Issue/通知不能被 Full Access 静默放行。五个 GUI mutation 已接入最长 5 秒、绑定明确 window/element selector 的可访问性 postcondition。多 Effect 复合语义 scope、截图式断言、失败恢复策略、macOS/installed E2E 仍开放，Credential Broker 也尚未完成 provider/project/session/operation/expiry 全链路作用域。
...
| P0-2 | 审批作用域与本地执行真相仍不完整 | Docker 产品模式已删除，历史 strict 设置 fail-closed；结构化权限规则已持久化并幂等迁移旧文本字段，可 AND 组合工具、路径、命令、网络 hostname、GUI process/window、普通或动态 MCP tool、MCP 非敏感标量参数、有效 postcondition 与风险条件。MCP 参数范围使用成对 RFC 6901 pointer/wildcard，敏感字段、无效 pointer、对象/数组值 fail-closed；动态工具保持 high risk 且 Full Access 不可静默放行。GUI 与一组稳定非 GUI 操作的五分钟 grant 已进入主进程内存控制面，绑定 session、项目 cwd、精确 tool/action、完整 input digest 和 frozen Effect target/diff digest，重启/关闭/到期可撤销，Renderer 只能读取/撤销。审批卡同时显示输入摘要与 Effect scope。high/critical 与 MCP/push/Issue/通知不能被 Full Access 静默放行。五个 GUI mutation 已支持绑定目标的可访问性 postcondition，`40/40` required 合同门禁、`11/11` Windows GUI 门禁、`p0-004`、MCP client 和真实 Windows Notepad observe-act-verify 已通过；最新 page smoke 为 `21/21`（`test-results/caogen-deep/2026-08-07T12-47-31-473Z/page-operation-smoke.json`）。多 Effect 复合语义、截图式断言、恢复策略、macOS/真实安装包 E2E 尚未闭环 | Codex sandbox/approval/Guardian 与 Claude permissions/OS sandbox 都把能力边界作为一等产品面 | 用户能准确知道“允许了什么、对哪个目标、持续多久” | 授权绑定 app/window/action/path/diff/postcondition，可随时撤销；UI、运行时和审计使用同一能力状态 |
...
| P1-10 | Desktop 自动化 observe-act-verify 仅完成第一阶段 | `gui_activate_window/click/type/scroll/hotkey` 已接受有界 postcondition，动作后轮询窗口/元素的 exists、absent、enabled、disabled、visible、hidden 状态，查询失败不能证明 absent，取消信号贯穿验证；真实 Windows Notepad 已验证 element visible 与关闭后 window absent。截图断言、补偿/恢复、macOS 和 installed E2E 仍缺 | 桌面自动化的有效基线是“观察 → 操作 → 验证”；当前实现已不再只有动作回执，但证据类型和平台覆盖尚不完整 | 减少“点了但没生效”“输入到了错误窗口”这类假成功 | 每个动作可声明 postcondition；执行后自动截图/读取可访问性状态并断言；失败可恢复；权限绑定具体 app/window；Windows/macOS 真实 E2E 进入 required gate |
...
| P2-6 | 内建代码工作台尚不能替代成熟 IDE，迁移价值缺真人证据 | 专用 VS Code/JetBrains 插件与 IDE Bridge 已退役；现有文件、Diff、终端、Git 和 Preview 仍缺 LSP、跨文件诊断/重构、测试导航与调试闭环 | 把外部 IDE 仅作为迁移来源和基准，不再为其开发 CaoGen 插件 | 降低现有 IDE Agent 用户切换成本，同时把代码交付留在 CaoGen 的权限、Effect、Artifact 和 Acceptance 边界内 | 在不安装 IDE 插件的前提下，重度用户完成大型仓库索引、跨文件修改、诊断修复、测试定位、调试和 Git 审查；30 分钟迁移资产零丢失并记录回退次数 |

2026-08-10 更新：项目树、全文/符号搜索、多标签编辑、TypeScript completion/hover/definition/diagnostics、syntax fallback diagnostics、Diff、终端、Git/worktree/PR、Preview 和受限测试运行入口已经接通。测试入口覆盖结构化退出、取消、超时、输出上限和脱敏耐久证据，专项 `26/26`、Electron `11/11`、文件编辑器回归 `29/29`。P2-6 的主要剩余缺口已收敛为断点调试、重构能力、大型仓库性能/崩溃恢复黄金门禁和真人重度任务，不再把 LSP 或基础测试入口列为未实现。

2026-08-07 的 P0-2 增量已关闭“单次工具调用缺少复合能力 envelope”这一子缺口：权限规则 v2 与 DigitalWorker 共用五类能力分类，allow 要求全集覆盖、deny 任一交集即阻断，`bash`、交互 GUI、普通/动态 MCP 均按复合能力处理；审批卡、运行时与审计使用同一主进程分类结果。最新 `p0-004`、GUI required `40/40`、DigitalWorker policy `122 checks`、MCP client、typecheck、production build 与 page smoke `21/21` 通过，页面证据为 `test-results/caogen-deep/2026-08-07T13-05-23-425Z/page-operation-smoke.json`。P0-2 行中仍开放的“多 Effect 复合语义”指跨多个 `EffectTarget` 的账本、对账和补偿；截图式断言、失败恢复、macOS/installed E2E 与 clean release 绑定仍未完成。

2026-08-07 的 P1-10 增量已关闭“截图式断言缺失”以及“精确可访问性后置条件无法在重启后只读确认”两个子缺口：五个 GUI mutation 可在动作前建立内存视觉基线，并用冻结的同一 window source 验证 `changed/unchanged`；非唯一目标、空帧、源/尺寸漂移、取消和超时均 fail-closed，安全证据只包含尺寸、变化比例和 digest。只有绑定精确 `windowId`（元素条件另要求精确 `elementId`）、动作前尚未成立且与动作目标一致的 window/element 后置条件才生成 `gui_postcondition` EffectTarget；重启后只读对账只能 `confirmed/unresolved`，不自动重放或补偿 GUI 动作。真实 Windows Notepad 最新报告为 `test-results/gui-input-preflight/2026-08-07T13-49-16-282Z/report.json`，其中恢复结果为 `confirmed`；GUI required 当前为 `49/49`。旧 window-absent 单次超时失败证据仍保留在 `2026-08-07T13-35-06-277Z`。视觉、模糊选择器、macOS、installed E2E、自动补偿和 clean release 绑定仍开放，`FIX-000 / No-Go` 不变。
...
- 旧 GUI 全局到期时间已经失效；当前 GUI 与稳定非 GUI grant 均由主进程按 session/cwd/tool/完整 input digest 签发、枚举、到期和撤销。权限设置页使用版本化结构化规则并兼容迁移旧文本字段，可 AND 组合命令、网络 hostname、GUI process/window、MCP tool 与有效 postcondition；非 GUI grant 额外绑定 frozen Effect target/diff digest，审批卡显示输入和 Effect scope。GUI grant 还要求稳定窗口/元素目标；非 GUI grant 拒绝 high/critical 与不透明外部副作用。剩余缺口是动态 MCP adapter、参数级/多 Effect 复合语义、全入口一致性和 installed/macOS 行为证据。
- Docker 产品模式和旧降级开关已删除；历史 strict 用户进入 `disabled` 确认态。剩余工作是把 capability grant、凭据和数据保留进一步统一到可解释控制面。
...
| P1-10 | GUI observe-act-verify、截图断言和精确可访问性 Effect 恢复已落地，平台与补偿边界未闭环 | 保持 visual/fuzzy 条件人工对账，补 macOS、installed required E2E；通用 GUI 不自动重放或反向补偿 | 假成功率可度量下降，真实 Windows/macOS required E2E 覆盖主链路 | Desktop Automation |
