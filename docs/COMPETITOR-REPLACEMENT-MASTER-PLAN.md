...
2026-08-11 representative-index increment: the large-repository gate now generates exactly 50,000 source files across TypeScript, TSX, JavaScript, Python, Go, Rust and Java, with language-specific tail sentinels sorted after the bulk tree. The verified Windows run indexed all 50,000 files and 78,571 symbols in `29.2s` with one persisted snapshot; every language tail symbol, the TypeScript dependency, complete substring/fuzzy lookup and empty code search passed. Persisted warm reopen completed in `2.51s`. A 50,001st file then produced `truncated=true`, preserved exactly 50,000 existing rows and excluded the beyond-ceiling symbol. Sanitized private evidence is `test-results/indexer-large-repo/2026-08-10T17-56-38.554Z.json`; `test:indexer`, typecheck, script syntax and diff checks also pass. This closes the representative mixed-language performance slice only; network-filesystem/sustained-watcher and real-Electron heavy-user evidence remain open.

2026-08-11 network-watcher increment: the indexer now detects UNC roots and uses bounded Chokidar polling instead of Windows native `fs.watch`, which emitted `EPERM/UNKNOWN` on SMB. Atomic SQLite replacement retries only bounded transient network-filesystem errors, while dispose rejects new queued updates before draining active work. The required Windows loopback SMB/UNC gate sustained 16 waves and 1,392 create/overwrite/rename/delete operations, converged to exactly 80 files and 160 symbols, emitted no watcher or persistence errors, left no temporary snapshots and reopened the persisted UNC database with the same final identities. Latest evidence is `test-results/indexer-network-watcher/2026-08-10T18-09-46.846Z.json`; the post-fix 50,000-file regression remains green at `29.3s` cold and `2.52s` warm. This closes the local SMB/UNC and sustained-storm slice, not remote-server disconnect, latency/jitter or a real-Electron heavy-user workflow.

2026-08-11 built-in IDE workflow increment: `test:ide-heavy-workflow:required` builds once and binds seven required phases to one commit/build: the 50,000-file mixed-language index, SMB/UNC sustained watcher, real Electron file/LSP/diagnostics/Diff/Git/terminal flow, tests, debugger, refactor and keyboard traversal. The file workflow now passes `36/36` in one Project Session, including search, completion, definition, semantic/syntax repair, visible Diff review, Stage all, in-app commit with a clean worktree and a project-scoped terminal command. The final aggregate report passes 7/7 phases, detects no newly launched VS Code/JetBrains/Visual Studio process and is stored at `test-results/ide-heavy-workflow/2026-08-10T18-45-01-712Z.json`, bound to implementation commit `2741dd944dae84d73676c4078312ace52d74d4d3`. This is an automated built-in capability gate across isolated Electron fixtures; it does not claim one continuous human task, remote SMB failure recovery or final 1.0 acceptance.

2026-08-11 UNC root-recovery increment: an already-open Windows loopback SMB project now detects that its root has disappeared, freezes incremental mutation and persistence, retains the last complete in-memory and durable SQLite snapshot, and performs a full transactional rebuild after the root returns. The required gate physically renames the local directory behind `\\localhost\C$`, creates a source file while it is offline, verifies the 80-file snapshot and database SHA-256 remain unchanged, restores the directory and converges to 81 files with the offline sentinel. The latest focused evidence is `test-results/indexer-network-watcher/2026-08-10T19-08-49.213Z.json`; typecheck and the 50,000-file regression also pass, with the latter at `29.37s` cold and `2.57s` warm. This covers local SMB root disappearance/recovery only, not a remote NAS or server outage, WAN latency/jitter, credential expiry, or a continuous human heavy-user task; `IDE-001/002` remain open.

The clean post-fix implementation commit `5b79291333890ab1d0524357e98a1ed3fe463c14` subsequently passed the complete built-in IDE required gate at `test-results/ide-heavy-workflow/2026-08-10T19-14-33-347Z.json`: all 7/7 phases passed, the 50,000-file run completed in `28.89s` cold and `2.56s` warm, UNC storm plus root disappearance/recovery converged from the retained 80-file snapshot to 81 files, and the Electron workflow remained `36/36 + 11/11 + 13/13 + 12/12 + 8/8`. No external IDE process appeared. This binds the outage fix to the full automated workbench evidence without changing the remaining human/remote-system acceptance boundary.
...
2026-08-11 Provider 401 recovery increment: the one-token generation preflight now returns a bounded diagnosis with the actual safe task pathname, protocol, credential source/header names, failure reason and ordered recovery actions. A 401/403 no longer overclaims that the entered key is wrong: the UI explicitly distinguishes invalid credentials or model permission from credential-header, Base URL path and protocol mismatches, then focuses the key or Base URL field or opens advanced protocol configuration. Provider origin, query, model identity, credential values, network details and response bodies never enter the result. The focused contract passes 12/12 status/privacy scenarios; real Electron passes 9/9 at `test-results/provider-generation-diagnostic-e2e/2026-08-10T19-34-46-980Z`, including the configured task request, three recovery paths, redaction and `760x700` containment. Provider restart regression passes 9/9 after replacing a broken Windows `npx.cmd` launcher and updating its stale assertion to require Base URL redaction. Typecheck, production build, connectivity and preset gates pass. This closes the actionable 401-diagnosis slice of `M2-T2`, not real-account authorization, provider-side model entitlement or human onboarding acceptance.
