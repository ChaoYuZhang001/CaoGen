...
const userManagedRoot = path.join(home, '.caogen')
const projectManagedRoot = path.join(project, '.caogen')
const userMcpPath = path.join(userManagedRoot, 'mcp', 'mcp.json')
const projectMcpPath = path.join(projectManagedRoot, 'mcp', 'mcp.json')
const userRulesPath = path.join(userManagedRoot, 'rules.md')
const projectRulesPath = path.join(project, 'caogen.md')
...
try {
  ...
  const settings = JSON.parse(readFileSync(userMcpPath, 'utf8'))
  ...
  const projectMcp = JSON.parse(readFileSync(projectMcpPath, 'utf8')).mcpServers
  ...
  const projectRules = readFileSync(projectRulesPath, 'utf8')
  ...
  const userRules = readFileSync(userRulesPath, 'utf8')
  ...
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'cline--project-debug', 'SKILL.md')), 'Cline project Skill target is namespaced')
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'gemini--project-planning', 'SKILL.md')), 'Gemini project Skill target is namespaced')
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'qwen--project-review', 'SKILL.md')), 'Qwen project Skill target is namespaced')
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'opencode--pr-review', 'SKILL.md')), 'OpenCode project Skill target is namespaced')
  assert(existsSync(path.join(userManagedRoot, 'skills', 'cline--global-review', 'SKILL.md')), 'Cline user Skill target is namespaced')
  assert(existsSync(path.join(userManagedRoot, 'skills', 'gemini--global-research', 'SKILL.md')), 'Gemini user Skill target is namespaced')
  assert(existsSync(path.join(userManagedRoot, 'skills', 'qwen--global-review', 'SKILL.md')), 'Qwen user Skill target is namespaced')
  assert(existsSync(path.join(userManagedRoot, 'skills', 'opencode--release-notes', 'SKILL.md')), 'OpenCode user Skill target is namespaced')
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'openclaw--shared', 'SKILL.md')), 'workspace Skill target is namespaced')
  assert(existsSync(path.join(projectManagedRoot, 'skills', 'openclaw-agents--shared', 'SKILL.md')), 'agents Skill target cannot overwrite workspace Skill')
  assert(existsSync(path.join(userManagedRoot, 'skills', 'hermes--writing--office--report', 'SKILL.md')), 'nested Hermes Skill target is deterministic')
  ...
  assert(!existsSync(userMcpPath), 'rollback removes newly created user MCP target')
  assert(!existsSync(projectMcpPath), 'rollback removes newly created project MCP target')
  assert(!existsSync(projectRulesPath), 'rollback removes newly created project rules target')
  assert(!existsSync(userRulesPath), 'rollback removes newly created user rules target')
  assert(!existsSync(path.join(projectManagedRoot, 'skills', 'openclaw--shared')), 'rollback removes newly created project Skill')
  ...
  const replaceTarget = path.join(userManagedRoot, 'skills', 'hermes--writing--office--report')
  ...
  rmSync(userManagedRoot, { recursive: true, force: true })
