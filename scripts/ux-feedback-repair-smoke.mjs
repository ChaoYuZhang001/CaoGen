...
const projectPickerCss = source('src/renderer/src/components/studio/ProjectWorkspaceStudio.css')
...
const assertions = [
  ...
  [project.includes('<ProjectPicker') && !project.includes('<select') && projectPicker.includes('data-project-workspace-select-trigger') && projectPicker.includes('data-project-workspace-select-menu') && projectPicker.includes('data-project-workspace-select-static') && projectPicker.includes('data-project-workspace-select'), 'Project selection uses an in-window picker and keeps the native bridge out of the visible surface'],
  [projectPickerCss.includes('.pws-project-select-menu') && projectPickerCss.includes('position: absolute') && projectPickerCss.includes('max-height: min(320px, calc(100vh - 120px))') && projectPickerCss.includes('overflow: auto') && projectPickerCss.includes('width: min(360px, calc(100vw - 32px))'), 'Project picker menu has bounded desktop dimensions and scrolls long project lists'],
