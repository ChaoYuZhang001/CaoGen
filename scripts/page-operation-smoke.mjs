...
async function waitForOfficeTelemetry(cdp, timeout = 10_000) {
  ...
  while (Date.now() - start < timeout) {
    last = await evalValue(
      ...
      `(() => {
        ...
        return {
          ...
          oneDigitalWorkerPerAgent: num('data-office-one-digital-worker-per-agent'),
          visibleDigitalWorkers: num('data-office-visible-digital-workers'),
    ...
    const ok = [
      ...
      last.oneDigitalWorkerPerAgent === 1, last.visibleDigitalWorkers === last.sessions, last.visibleRobots === 0,
