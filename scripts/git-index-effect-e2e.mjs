...
function stageSelectedCase(indexEffect) {
  ...
  const execution = indexEffect.executeGitIndexEffectTarget(target, input)
  assert(execution.ok, `stage_paths execution failed: ${JSON.stringify(execution)}`)
...
function objectDirectorySymlinkCase(indexEffect) {
  ...
  const before = indexDigest(repo)
  symlinkSync(outside, objectPrefixPath, process.platform === 'win32' ? 'junction' : 'dir')
  const result = indexEffect.executeGitIndexEffectTarget(target, input)
