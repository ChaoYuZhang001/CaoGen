...
try {
  ...
  execFileSync(electron, [runner], {
    ...
    env: {
      ...process.env,
      CAOGEN_PROJECT_DEBUG_PANEL_ROOT: tempRoot,
      CAOGEN_PROJECT_DEBUG_PANEL_STATE: statePath,
      CAOGEN_PROJECT_DEBUG_PANEL_SCREENSHOTS: reportDir
    }
  })
  const report = JSON.parse(readFileSync(statePath, 'utf8'))
  if (!report.ok || report.pass !== report.total || report.total < 12) {
    throw new Error(`project debug panel E2E incomplete: ${JSON.stringify(report)}`)
  }
  console.log(`project debug panel E2E ok: ${reportDir}`)
