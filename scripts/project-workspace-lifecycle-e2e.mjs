...
import { assertProjectResource, sha256Hex, verifyDeletionProofDigest, verifyProjectExportDigest } from './lib/project-workspace-lifecycle-assertions.mjs'
import { verifyBoundedProjectPicker } from './lib/project-workspace-lifecycle-picker.mjs'
...
try {
  await runPhase('create-edit-archive', async (page) => {
    ...
    await check('single Project is static while the multi-Project picker stays bounded and restores focus',
      () => verifyBoundedProjectPicker(page, screenshot))
...
async function verifyGoalTaskRetry(page) {
  ...
  const digest = sha256Hex(`caogen.project-goal-task.v1\0${state.projectId}\0${requestId}`).slice(0, 24)
