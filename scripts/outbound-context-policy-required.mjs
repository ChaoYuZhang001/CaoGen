...
function assertProductionBoundaries() {
  ...
  check('retry and failover paths retain policy rechecks', () => {
    ...
    assert(openai.includes('providerAllowedByOutboundContext(this.activeOutboundContext'), 'OpenAI failover filter missing')
    assert(anthropic.includes('providerAllowedByOutboundContext('), 'Anthropic failover filter missing')
  })
