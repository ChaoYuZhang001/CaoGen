...
function assertRuntimeMonthlyBudgetWiring() {
  ...
  assert(openaiEngine.includes('calculateDurableMonthlyBudgetSnapshot'), 'native auto routing must use the durable monthly budget')
  assert(openaiEngine.includes("rootDir: app.getPath('userData')"), 'native auto routing must read the user-scoped billable ledger')
  assert(openaiEngine.includes('settings.budgetUsdPerMonth > 0'), 'native auto routing must skip ledger reads when monthly budget is unlimited')
  assert(openaiEngine.includes('monthlyBudgetRemainingUsd: monthlyBudget.remainingUsd'), 'native auto routing must pass monthly remaining budget')
  const sessionManager = read('src/main/sessionManager.ts')
  assert(sessionManager.includes('calculateDurableMonthlyBudgetSnapshot'), 'send gate must use the durable monthly budget')
