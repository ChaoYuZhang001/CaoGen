...
function verifyProductionWiring() {
  ...
  assert.match(coordinator, /store\.assertExecutionAuthorized\(this\.executionAuthoritySessionId\(meta\), false\)/)
  assert.match(coordinator, /store\.assertExecutionAuthorized\(this\.executionAuthoritySessionId\(session\.meta\), false\)/)
  assert.match(coordinator, /if \(visited\.has\(current\.id\)\) throw new Error\('任务计划父会话链形成循环，已阻止执行'\)/)
  assert.match(coordinator, /if \(!parent\) throw new Error\('任务计划父会话不可用，已阻止子任务执行'\)/)
