...
- 等待测试者期间继续关闭首任务恢复缺口：欢迎页草稿现在以版本化本地记录保存提示词、项目选择、目录、Drive、权限、路由、Provider ID 与模型，renderer reload 或应用重启后可恢复；成功创建首个任务后自动删除。记录不包含 API Key、Provider Base URL 或模型响应，损坏/超限记录会丢弃，存储不可用时退化为原内存草稿。真实 Electron `routing-zero-choice` 报告 `2026-07-26T18-24-01-671Z` 为 `10/10`、7 张截图，其中 reload 前后八项草稿字段一致、会话数保持 0、本地记录不含测试 Key/Base URL，首任务成功后记录已删除；页面操作回归 `2026-07-26T18-20-01-333Z` 为 `22/22`。该用户可见修复晚于 `837f8f90` 候选，因此下一次发布候选必须重跑。
