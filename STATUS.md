...
> `0.1.8` Intel-only 未发布候选 `main@03c3fee2837d120fce43f4b7d11bd25488be4d36` 的只读 run [`30243108279`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30243108279) 已覆盖 Browser/Preview 发送可靠性后继，并通过 source/P2、clean Deep、签名、公证、安装、renderer 与五资产绑定。Actions artifact `8644829708` 已用可续传 Range 独立重组并核验 `255,552,267 B`、GitHub ZIP SHA-256 `ec343fe823c5e3a3502c4b6176d23dd59b46dd3e15c2338b06eab98f7384c16a`、五资产集 digest `2abe8622e3b37873e69abdd5deb1f16c8739336181688eeb2e665c601792ff52`、更新元数据、纯 x86_64、包内 provenance、本地 codesign/Gatekeeper/应用 stapled ticket 及 Claude SDK/CLI 缺失。`1707a999 / 30235833246` 只保留为历史签名证据。精确 Final Notes 和四类报告已在 clean、已推送的 publication-only 后继上通过 scoped notes/handoff preflight，结论为 `ready_for_owner_decision`，所有副作用均为 false；当前仍没有 v0.1.8 发布授权，也未创建 tag/Release。v0.1.7 仍是最新公开版，也不是正式 1.0 stable：PRD 64 个 P0 = 21 个已验证 + 18 个部分完成 + 24 个立项目标 + 1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；0.1.8 只允许交付已验证补丁，不把路线图能力写成已完成。
>
...
- v0.1.7 tag 后已有用户可见修复进入 `main`，但公开安装包不包含它们；因此按 `docs/PLAN.md` 的 patch 规则启动 `0.1.8` macOS Intel-only 候选。版本与候选合同锁到 `0.1.8` / `macos-x64`，手动证据 workflow 保持 `contents: read`；Apple Silicon/Windows 不恢复，v0.1.7 远端正文与 3 个 Windows 资产不修改。下述 run 已关闭当前产品提交的候选构建、独立下载、Final Notes 与 clean scoped notes/handoff preflight；当前停在 `ready_for_owner_decision`，仍须获得新的明确发布授权。
...
**P0**
- 用户反馈快修循环(常设)
- v0.1.8 Intel-only 补丁候选：`03c3fee2 / 30243108279` 已完成 clean Deep、签名公证、安装启动、精确五资产、独立下载及 clean scoped notes/handoff preflight；状态为 `ready_for_owner_decision`，不含 tag/Release 发布权限
...
| v0.1.8 Intel 补丁候选 | High | 候选证据与 clean publication-only scoped notes/handoff preflight 均通过，状态 `ready_for_owner_decision`；仍需新的明确发布授权，不得发布 |
