...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 Intel x64 v0.1.7 楔子版。** 删除 Claude Runtime 后的 Intel 签名/公证/安装与五资产绑定主链已在历史候选 run `30150795350` 闭环；Release Notes scoped final gate 也已在 clean 后继提交上通过。M1-T6 的只读发布交接预检和公开资产 digest 门禁已经实现，`8ae1e207` 的本地 exact-commit Deep 已 `157/157` required 全绿，但 GitHub macOS runner internal error 与连续 0-job workflow failure 阻断了新签名/公证候选。下一步以记录本地 Deep 与基础设施失败的 docs-only exact commit 再运行一次 Intel 候选；新候选通过后，才只剩获得明确授权后的 Tag/Release、公开资产审计和官网同步。Apple Silicon 与 Windows 按创始人决定暂停，不计入当前 M1 阻塞项，也不得写成完成。当前候选仍未发布，未经明确发布动作不建 tag/Release。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
...
| 新最终候选 Deep | High | ⏳ `8ae1e207` 本地 Deep 已 `157/157` required 全绿、2 optional skip；GitHub run `30157730258` 过源码门禁后遇到 runner internal error，后续 `30157949745` / `30158101444` 均为 0-job 平台失败。仍需新 exact-commit GitHub Intel 候选，optional skip 不得计作 pass |
