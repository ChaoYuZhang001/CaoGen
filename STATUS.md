...
> `0.1.8` Intel-only 未发布候选 `main@837f8f90945d558c44b2d05cbc09a24e93d1202f` 已通过 clean Deep、签名、公证、安装、renderer 与五资产绑定，但当前源码又增加了首任务重启恢复修复，因此该候选只保留为精确提交的历史证据，发布最新源码前必须在后继 clean `main` 上重跑。尚未创建 v0.1.8 tag 或 Release。v0.1.7 仍是最新公开版，也不是正式 1.0 stable：PRD 64 个 P0 = 21 个已验证 + 18 个部分完成 + 24 个立项目标 + 1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；0.1.8 只允许交付已验证补丁，不把路线图能力写成已完成。
>
...
- v0.1.7 tag 后已有用户可见修复进入 `main`，但公开安装包不包含它们；因此按 `docs/PLAN.md` 的 patch 规则启动 `0.1.8` macOS Intel-only 候选。版本与候选合同锁到 `0.1.8` / `macos-x64`，手动证据 workflow 保持 `contents: read`；Apple Silicon/Windows 不恢复，v0.1.7 远端正文与 3 个 Windows 资产不修改。下述 run 已关闭候选构建验证，发布前仍需独立下载核验、Final Notes、publication-only 预检与新的明确授权。
- `main@837f8f90945d558c44b2d05cbc09a24e93d1202f` 的 0.1.8 Intel-only 候选 run [`30212121353`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30212121353) 已成功：source gates 与 P2 通过，exact-commit Deep 为 `163 total / 161 required pass / 2 optional skip / 0 blocked / 0 fail`，macOS audit 为 `120/120`，Developer ID 签名、公证/staple、Gatekeeper、DMG 隔离安装/干净 detach、真实 renderer 和 packaged-app smoke 全部通过。五资产集 digest 为 `48667aeb2f5bb2e16187e88c53a5db96d448d9cfa94e8c3afcfcaf561d510ed1`；Apple Silicon、Windows 和完整矩阵汇总均按范围跳过。Actions artifact `caogen-release-macos-x64-837f8f90945d558c44b2d05cbc09a24e93d1202f` 保存候选资产与报告，到期日为 2026-08-09。该结果只证明未发布 Intel 候选，不授权 tag、Release、v0.1.7 远端修改，也不关闭真人 M1 或 formal 1.0 门禁。
- 等待测试者期间继续关闭首任务恢复缺口：欢迎页草稿以版本化本地记录保存提示词、项目选择、目录、Drive、权限、路由、Provider ID 与模型，renderer reload 或应用重启后可恢复；成功创建首个任务后自动删除。启动时 Project/Provider 目录尚未完成异步加载，不再把空列表误判为资源已删除；目录加载完成后，真正删除、无密钥或无模型的 Provider 会 fail-closed 解绑，固定模型失效会清空模型，不会静默切换到其他 Provider。用户可从真实 Provider 编辑器重建配置，提示词、项目和其余草稿字段保持不变。记录不包含 API Key、Provider Base URL 或模型响应，损坏/超限记录会丢弃，存储不可用时退化为原内存草稿。真实 Electron `routing-zero-choice` 报告 `2026-07-26T18-41-31-736Z` 为 `11/11`、8 张截图；页面操作回归 `2026-07-26T18-42-00-814Z` 为 `22/22`。两项结果来自当前功能工作树，不是 clean 候选或真人验收；该用户可见修复晚于 `837f8f90` 候选，因此下一次发布候选必须重跑。
