...
- M1 首位陌生用户验收工具链已就绪：`docs/M1-FIRST-USER-DRILL.md` 固定官网→Intel DMG→无安全绕过安装→Provider 配置→只读任务五步；私有结果模板和 `test:m1-first-user-onboarding:required` 绑定 `v0.1.7`、候选提交、公开 DMG SHA-256、Intel 架构、30 分钟、零修改及四份独立证据。失败演练可用 `--observation` 留下 `observed_failed` 记录，但不会成为通过。负向 smoke 已覆盖超时、卡点、错误哈希、安全绕过、arm64、敏感字段、重复/符号链接证据。当前仍没有非项目参与者真人结果，M1 整体完成判据保持开放。
- M1 招募已公开：`npm run prepare:m1-first-user-drill` 会在仓库外创建权限收紧的私有目录、记录模板和主持人清单，不下载 DMG、不创建假证据；中英文 README、[Discussion #9](https://github.com/ChaoYuZhang001/CaoGen/discussions/9) 和 `CaoGen-Website@f8e8c50` 生产下载区只征集 Intel 机型、时区和可参与时间，并明确禁止公开 Key、Provider URL、项目路径、录屏或证据。官网中英文页面均已实测返回该入口，在线社区审计为 1 条维护者 Discussion、0 pending / 0 overdue / 0 late。当前合格参与者和真人记录仍为 0，不能关闭 M1。
...
- M2-T2 onboarding 应用内基础已在 v0.1.7 后续源码完成：欢迎页首张卡精确注入公开 `quick_start_project_read_only_v1` 只读提示词；Assistant 无执行资源与 Expert 无 Provider 都提供可操作恢复，配置按钮直达 Providers 页，重试会刷新 Provider 并清除恢复态。真实 Electron `routing-zero-choice` E2E `9/9` 覆盖提示词精确值、零误建会话、双模式恢复、设置深链、Provider 刷新、真实 Router/流式响应及模式切换状态不变；编码标准 ratchet 同时把 `WelcomeView` 复杂度从 20 降到 18、主 store 从 4012 行降到 4007 行。脏工作树完整 Deep `2026-07-25T18-20-19-937Z` 为 `160 total / 158 required pass / 2 optional skip / 0 blocked / 0 fail`；性能门禁只移到 build 后优先测量以避免整套长测的累计负载污染，冷/热 P95 为 `243.80ms / 263.00ms`，原 `<300ms` 阈值、样本与断言均未放宽。该结果不代表 clean release 绑定、M1 真人验收或 M2-T2 完成，仍需用首位陌生用户和三人 N1 记录验证配置成功率、文案理解与恢复时长。
...
- M2-T3 反馈通道基础已上线：公开 GitHub Discussions 已启用并保留 General、Ideas、Q&A 分类，仓库提供双语分类表单、Issue/README/SUPPORT 导流和明确的 48 小时维护者首次回应口径；只读定时工作流每 6 小时扫描公开 Issue、PR、Discussion，OWNER/MEMBER/COLLABORATOR 回应才计数，自动回复不计数，当前逾期会阻断、历史迟复会告警。当前还没有真实外部反馈，入口理解度与实际 SLA 尚未形成用户证据，因此 M2-T3 仍为部分完成。
