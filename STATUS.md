...
>
> ⚠️ **未达完整发布标准**。`main@e8f617f822be458065dabe7f2440d1f5a33ee2b3` 的 Intel-only 候选曾通过精确提交四态 Deep、Developer ID 签名、公证、staple、Gatekeeper、DMG 隔离安装、真实 renderer 启动、五资产更新元数据绑定和 clean 后继提交 scoped final-notes audit；本轮发布预检与公开资产摘要审计改变了源码，因此该候选现为历史证据，必须在新提交上重跑 Intel 候选。Apple Silicon 与 Windows 按创始人决定暂停，不能写成已完成，完整三平台 Release Doctor 因此仍为 `not_ready`。当前未创建 tag/GitHub Release。Docker 已从产品模式删除；正式运行时只保留 OpenAI-compatible 与原生 Anthropic Messages，不需要 Claude Code 登录、SDK 或 CLI。
...
- `main@e8f617f822be458065dabe7f2440d1f5a33ee2b3` 的 Intel-only GitHub 最终候选 run [`30150795350`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30150795350) 已成功：release-scope P2 gate 通过，exact-commit Deep 为 `157 total / 155 required pass / 2 optional skip / 0 blocked / 0 fail`，x64 完成 Developer ID 签名、公证、staple、Gatekeeper、`120/120` required release audit、DMG 隔离安装、干净 detach、真实 renderer 启动，以及 `latest-mac.yml` 对 x64 DMG/ZIP 的名称、版本、大小和 SHA-512 绑定。该 run 的 Apple Silicon、Windows 与完整矩阵汇总 job 均按输入范围跳过，因此只证明 Intel，不证明完整三平台。
...
- `.github/workflows/release-candidate-evidence.yml` 已把 M1-T3 固化为只读、手动、不可发布的候选证据管线：输入必须是已在 `main` 的完整 40 位 SHA 与精确版本；可选择 `macos-x64` 或完整三平台范围。run `30150795350` 已证明仓库中的 Intel 签名/公证 secrets 可用；这不证明 Apple Silicon/Windows secrets 或资产。workflow 不创建 tag/GitHub Release。
- 移除 Claude Code Runtime 前的本地 unsigned Intel 基线为 DMG `197,518,789 B`、ZIP `197,966,775 B`、`.app` 逻辑体积 `555,506,427 B`、`app.asar` `61,742,511 B`，其中 Claude CLI 为 `240,192,080 B`。移除后的本地 unsigned Intel 实测为 DMG `125,638,991 B`、ZIP `125,002,021 B`、`.app` 逻辑体积 `311,931,918 B`、`app.asar` `61,630,882 B`，分别减少 `36.39% / 36.86% / 43.85% / 0.18%`。最终 clean candidate 的签名公证资产为 DMG `127,703,535 B`（SHA-256 `24bb2e4d6d98987955495b51fa4d6a8feda9634b923a3fa9e5f66f9fd60f0d9b`）与 ZIP `127,016,726 B`（SHA-256 `1e668a18dfb758b20dcadadf23b6a95433d6c4c8197a753738a54ba33722af45`）；包含两项 blockmap 与 `latest-mac.yml` 的五文件资产集绑定 `0fc626d9ccc3038ca5b3e574b87692f5ec7778c0670b2a6a4139fd804dba216d`。required audit 证明 SDK/CLI 在包内不存在，且 app、DMG、ZIP 的 `0.1.7`、clean worktree 与完整提交 provenance 一致。
...
- 正式运行时只有 OpenAI-compatible（Responses / Chat Completions）与原生 Anthropic Messages（`/v1/messages`）两条执行路径，均已注册到 Provider/SessionManager/UI。Claude Code Agent Runtime、Claude Agent SDK 与 Claude CLI 已从依赖、主进程、IPC/UI、测试宿主和打包规则删除；旧 `engine: claude` Provider/会话只迁移为 `anthropic`，不恢复 SDK 隐藏上下文。历史 Intel 候选 Deep 报告 `2026-07-25T08-23-47-652Z` 为 `157 total / 155 required pass / 2 optional skip / 0 blocked / 0 fail`，开始和结束均绑定 clean `e8f617f8`；它只证明该精确提交的 Intel 候选和仓库 required gates，不替代当前源码的新候选、暂停平台、真实 Provider、N1 或完整 1.0 验收证据。
- M1-T6 已新增只读、fail-closed 的 Intel 发布交接预检：它绑定候选提交、四类报告、五项本地资产、Release Notes、clean 后继提交、远端 `main`，并要求本地/远端 tag 与 GitHub Release 均不存在；全部通过后才输出需创始人明确授权的手工发布命令，脚本自身不创建 tag、Release 或上传资产。发布后公开资产审计现要求每个 GitHub 资产的名称、大小与平台返回的 SHA-256 digest 精确匹配本地候选文件，并拒绝额外本地文件。`8ae1e207` 的本地完整 Deep 报告 `2026-07-25T12-37-28-261Z` 为 `159 total / 157 required pass / 2 optional skip / 0 blocked / 0 fail`，开始与结束均为 clean 同一提交，新增两项发布门禁均在 required 列表通过。`d5f0a86f` 的 Intel-only run `30157730258` 已通过候选身份与全部源码门禁，但 macOS runner 在 `npm ci` 期间被 GitHub Actions internal error 取消；后续同 SHA 重跑、run `30157949745` 和新 concurrency key run `30158101444` 均在 0 jobs、无仓库命令/日志时被平台判为 workflow failure。`bd26eaa3` 的 run [`30159248949`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30159248949) 已恢复真实执行，身份、源码、Intel 依赖/凭据和 release-scope P2 均通过，但 exact-commit Deep 的 `ModelAttempt crash reconciliation E2E` 暴露测试 worker 仅等待 pending Promise、事件循环可先于父进程 `SIGKILL` 退出的竞态；签名、公证和打包按门禁跳过。该测试夹具已改为用活动定时器保持 worker 存活，本地 targeted E2E 连续 `20/20` 通过；仍需把修复绑定到新 exact commit 并取得 GitHub Intel 签名/公证候选。
...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 Intel x64 v0.1.7 楔子版。** 删除 Claude Runtime 后的 Intel 签名/公证/安装与五资产绑定主链已在历史候选 run `30150795350` 闭环；Release Notes scoped final gate 也已在 clean 后继提交上通过。M1-T6 的只读发布交接预检和公开资产 digest 门禁已经实现，`8ae1e207` 的本地 exact-commit Deep 已 `157/157` required 全绿。GitHub 平台故障消退后，run `30159248949` 在 required Deep 暴露 ModelAttempt 强杀测试夹具的事件循环竞态；产品恢复断言没有被降级或跳过，worker keepalive 修复已本地连续 `20/20` 通过。下一步把该修复推到 `main`，立即运行新 exact-commit Intel 候选；新候选通过后，才进入发布交接预检，并等待明确授权后的 Tag/Release、公开资产审计和官网同步。Apple Silicon 与 Windows 按创始人决定暂停，不计入当前 M1 阻塞项，也不得写成完成。当前候选仍未发布，未经明确发布动作不建 tag/Release。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
...
| 新最终候选 Deep | High | ⏳ `8ae1e207` 本地 Deep 已 `157/157` required 全绿、2 optional skip；GitHub run `30159248949` 恢复执行后在 required ModelAttempt crash E2E 暴露测试 worker 事件循环竞态，签名/公证未运行。keepalive 修复本地 targeted 连续 `20/20` 通过；仍需新 exact-commit GitHub Intel 候选，optional skip 不得计作 pass |
