...
> `0.1.8` Intel-only 未发布候选 `main@1707a9990b7c922dd96986c372debfad99da36e9` 的只读 run [`30235833246`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30235833246) 已通过 source/P2、clean Deep、签名、公证、安装、renderer 与五资产绑定；Actions artifact `8642130786` 也已用独立 Range 下载重组并核验 GitHub ZIP digest `e4d616537676a461ba40725319a9ec422e4f02f6ad2092ed1fce456a5f980107`、五资产集 digest `756968f38c312b4df2dd9d4b53421ca838c7eec9ff5aab2153d07ec59e321b28`、更新元数据、包内 provenance 和本地 codesign/Gatekeeper/staple。当前源码随后又修复 Browser 批注发送失败被吞掉及 closed/undefined 会话仍可点击的问题，因此 `1707a999` 只保留为该精确提交的历史签名证据，不能发布为最新源码。尚未完成新一轮候选、Final Notes、publication-only 预检或 v0.1.8 发布授权，也未创建 tag/Release。v0.1.7 仍是最新公开版，也不是正式 1.0 stable：PRD 64 个 P0 = 21 个已验证 + 18 个部分完成 + 24 个立项目标 + 1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；0.1.8 只允许交付已验证补丁，不把路线图能力写成已完成。
>
...
- v0.1.7 tag 后已有用户可见修复进入 `main`，但公开安装包不包含它们；因此按 `docs/PLAN.md` 的 patch 规则启动 `0.1.8` macOS Intel-only 候选。版本与候选合同锁到 `0.1.8` / `macos-x64`，手动证据 workflow 保持 `contents: read`；Apple Silicon/Windows 不恢复，v0.1.7 远端正文与 3 个 Windows 资产不修改。下述 run 已关闭精确提交的候选构建验证和独立下载核验，但当前源码已有后继用户可见修复；发布最新源码前仍需重跑候选、Final Notes、publication-only 预检与新的明确授权。
- `main@1707a9990b7c922dd96986c372debfad99da36e9` 的 0.1.8 Intel-only 候选 run [`30235833246`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30235833246) 已成功：source gates 与 P2 通过，exact-commit Deep 为 `163 total / 161 required pass / 2 optional skip / 0 blocked / 0 fail`，macOS audit 为 `120/120`，Developer ID 签名、公证/staple、Gatekeeper、DMG 隔离安装/干净 detach、真实 renderer 和 packaged-app smoke 全部通过。五资产集 digest 为 `756968f38c312b4df2dd9d4b53421ca838c7eec9ff5aab2153d07ec59e321b28`；Apple Silicon、Windows 和完整矩阵汇总均按范围跳过。Actions artifact `8642130786` / `caogen-release-macos-x64-1707a9990b7c922dd96986c372debfad99da36e9` 到期日为 2026-08-10；其 `255,556,979 B` ZIP 已独立分段下载、按精确偏移重组并复算 SHA-256 `e4d616537676a461ba40725319a9ec422e4f02f6ad2092ed1fce456a5f980107`，五项资产大小/SHA-256、`latest-mac.yml` SHA-512、provenance、x86_64、codesign、Gatekeeper、staple 及 Claude SDK/CLI 缺失均与报告一致。`9591c20b / 30215660873` 降为更早历史证据；本组也已被后继 Browser 修复超越，不能直接发布，不授权 tag、Release、v0.1.7 远端修改或关闭真人 M1/formal 1.0 门禁。
...
**P0**
- 用户反馈快修循环(常设)
- v0.1.8 Intel-only 补丁候选：`1707a999 / 30235833246` 已完成 clean Deep、签名公证、安装启动、精确五资产及独立下载核验，但已被后继 Browser 可靠性修复超越；等待本轮功能修复收口后重跑，只准备候选，不含 tag/Release 发布权限
...
| v0.1.8 Intel 补丁候选 | High | 进行中：`1707a999 / 30235833246` 的全链证据和独立资产核验已通过，但后继 Browser 修复使其成为历史候选；新源码仍需 clean Deep 和新 Intel 候选，不得发布 |
