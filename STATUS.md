...
> `0.1.8` Intel-only 未发布候选 `main@9591c20bde2330a6f57951e7381b5e7e9d642091` 的只读 run [`30215660873`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30215660873) 已通过 clean Deep、签名、公证、安装、renderer 与五资产绑定，候选资产也已独立下载并逐项核验；旧 `837f8f90 / 30212121353` 只保留为更早历史证据。当前源码随后又修复固定模型切换 Drive 时被错误清空的问题，因此 `9591c20b` 也只保留为精确提交的历史签名证据，发布最新源码前必须重跑候选。尚未完成 Final Notes、publication-only 预检或 v0.1.8 发布授权，也未创建 tag/Release。v0.1.7 仍是最新公开版，也不是正式 1.0 stable：PRD 64 个 P0 = 21 个已验证 + 18 个部分完成 + 24 个立项目标 + 1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；0.1.8 只允许交付已验证补丁，不把路线图能力写成已完成。
>
...
- `main@9591c20bde2330a6f57951e7381b5e7e9d642091` 的 0.1.8 Intel-only 候选 run [`30215660873`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30215660873) 已成功：source gates 与 P2 通过，exact-commit Deep 为 `163 total / 161 required pass / 2 optional skip / 0 blocked / 0 fail`，macOS audit 为 `120/120`，Developer ID 签名、公证/staple、Gatekeeper、DMG 隔离安装/干净 detach、真实 renderer 和 packaged-app smoke 全部通过。五资产集 digest 为 `b5e03719796ea3236fab617c8e1493a238e3a07e48552daa1dc74b04f7d27252`；Apple Silicon、Windows 和完整矩阵汇总均按范围跳过。Actions artifact `8636056669` / `caogen-release-macos-x64-9591c20bde2330a6f57951e7381b5e7e9d642091` 保存候选资产与报告，到期日为 2026-08-09。五项资产已下载到仓库外临时目录并独立复算大小、SHA-256 与资产集 digest，结果和 audit/packaged-app 报告一致。该结果只证明该精确提交的未发布 Intel 候选；当前源码已有后继修复，不能直接发布这组资产，也不授权 tag、Release、v0.1.7 远端修改或关闭真人 M1/formal 1.0 门禁。
...
- 等待测试者期间修复固定模型与 Drive 切换冲突：Studio 首任务已明确选择有效 Provider/模型时，切换 Drive 只更新 Drive 与默认权限，不再把固定模型改成自动路由并留下空模型；若模型确实已从 Provider 目录消失，仍 fail-closed 清空。真实 Electron `routing-zero-choice` 报告 `2026-07-26T20-07-47-489Z` 为 `12/12`、9 张截图，页面操作 `2026-07-26T20-08-40-109Z` 为 `22/22`，build/typecheck 通过。该修复晚于 `9591c20b` 候选，当前仍需 clean Deep 和新的 Intel 候选。
- 等待测试者期间修复运行中重复发送的草稿丢失与假排队：Composer 现在允许继续起草，但任务处于 `starting/running` 时禁用发送并明确提示任务完成后再发；Browser 批注和产物 Preview 的发送入口也遵守同一忙碌门禁。只有主进程确认接受消息后才清空文本和附件；被执行引擎拒绝的消息会撤回乐观记录、恢复原会话状态并保留草稿，IPC 与 IDE Bridge 返回真实 `boolean`，记忆建议/自动提取只在确认接受后触发。真实 Electron live-switch 报告 `2026-07-27T03-28-37-915Z` 为 `6/6`：10 次 Assistant/Studio 往返不丢草稿，运行中 Enter 不发送且模型请求始终只有 1 个，底层重复发送返回 `false`，任务结束后发送按钮恢复；页面操作报告 `2026-07-27T03-26-54-546Z` 为 `22/22`，连续 Browser/Preview 发送会等待真实可发送状态，不再把拒绝当成功。build、typecheck、IDE Bridge、模型切换策略、Effect close-race 和编码标准 required gate 均通过；这仍是 targeted 证据，提交后的 clean Deep 与新 Intel 候选尚未完成，也不替代真人 M1。
