...
> 更新:2026-07-14(第 29 次)· 实测口径,非文档自评。此文件为活文档,Current Focus 随日更新。
>
...
- **[v0.1.5 macOS x64 与 Windows x64 已发布](https://github.com/ChaoYuZhang001/CaoGen/releases/tag/v0.1.5)**(2026-07-14)——GitHub 当前列出 8 个安装包、归档、blockmap 和更新元数据资产;tag 精确指向 `d9969e3`,macOS Intel 主二进制为 `x86_64`;macOS arm64/Linux 不在本轮发布范围
...
- GitHub Releases 公开资产审计分两段留证:v0.1.5 原 macOS 5 资产已通过 `--expected-assets-from-dist` 的名称、大小、状态、SHA256 与 `latest-mac.yml` 文本审计;Windows 资产加入后,2026-07-14 的远端清单复核确认共 8 个允许资产且 SHA256 digest 可读,但本次 `latest-mac.yml` / `latest.yml` 文本下载超时,因此不宣称组合后的 8 资产 read-text 已通过,发布前必须重跑。
- v0.1.5 最终发布说明保存在 `docs/RELEASE-NOTES-FINAL.md`,已在精确 release commit `d9969e3` 的干净工作树上通过 `npm run test:release-notes-audit:final`,并作为 GitHub Release 正文发布。
- Packaging gate 的 v0.1.5 macOS x64 DMG/zip、两个 blockmap 与 `latest-mac.yml` 已全部发布,资产集 SHA256 为 `15990fe33a095305021b996ae88a7ff9b54345606a88569a2fdfd4c58fd6ca65`。macOS 包仍未签名,Release notes 保留首次打开说明。
...
**v0.1.6 稳定修复发布 + Trust Kernel 继续收口**:先把 v0.1.5 后的会话/项目修复、首个 AGPL 发布身份、真实公开资产清单和 macOS x64 安装包闭环;专项回归必须覆盖“删空后新建可输入、3D 新建、项目归档/恢复/删除、未关联项目会话、开工建议仅手动打开、三种调度范围”。发布完成后按 PR/Issue/消息/可查询 MCP/Code Forge/Renderer 入口 → append-only evidence/compensation → GUI/工具审批作用域 → Base64 fallback 与 scoped credential broker → MCP/插件 Capability Manifest 的顺序继续。真实 China network/tool-call parity 由用户配置，N1 真人 30 分钟记录只阻塞 N1 达标宣称。
