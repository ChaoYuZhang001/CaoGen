...
- **v0.1.4 已发布**(2026-07-12,[GitHub Release](https://github.com/ChaoYuZhang001/CaoGen/releases/tag/v0.1.4),本轮只发布 macOS x64 的 DMG/zip、两个 blockmap 与 `latest-mac.yml`)——Intel 主二进制与实际启动已验证,arm64/Windows/Linux 不在本轮发布范围
- 正式运行时保留可选 Claude Agent SDK 与默认 OpenAI-compatible API 两类执行路径。最新完整四态门禁目标与当前基线为 `87 total / 84 required pass / 3 optional skip / 0 blocked / 0 fail`；本次不可变报告为 `test-results/caogen-deep/2026-07-11T23-32-36-183Z/deep-test-report.md`，滚动入口为 `test-results/caogen-deep/latest.md`。3 个 optional skip 分别保留真实 Claude、China real-network、China tool-call parity 的外部条件，不计入 pass，也不阻塞默认发布档位。
...
- GitHub Releases 公开资产审计已补齐:v0.1.4 发布后已通过 `npm run test:github-release-audit:read-text:required -- --tag v0.1.4 --expected-assets-from-dist`,线上 5 个资产的名称、大小、状态与 SHA256 均和本地 `dist` 一致,`latest-mac.yml` 也已读取;通过报告为 `test-results/github-release-audit/2026-07-12T00-03-53-543Z/report.json`。
- v0.1.4 最终发布说明保存在 `docs/RELEASE-NOTES-FINAL.md` 并已原样用作 GitHub Release 正文;该正文在精确 release commit `a14c623` 的干净工作树上通过 `npm run test:release-notes-audit:final`,滚动草稿保存在 `docs/RELEASE-NOTES-DRAFT.md`。
- Packaging gate 已完成:旧 `dist/` 历史包已归档到 `test-results/release-packaging-audit/archived-dist-*`,Electron 40 + `tree-sitter` 的原生模块构建阻塞已通过 `scripts/prepare-native-build.cjs` 修复;v0.1.4 的 macOS x64 DMG/zip、两个 blockmap 与 `latest-mac.yml` 已由 `npm run dist:mac:x64` 生成、验证并发布,资产集 SHA256 为 `452c26c741fa24ffdeaeffaf019c4fc4eecb72cab8f6a00111d4be8e5a6063b5`。macOS 包仍未签名,Release notes 保留首次打开说明。
...
- 产品定位门禁新增: `npm run test:product-positioning:required` 会扫描 README、欢迎页入口文案、Release notes、Release gate 和公开品牌入口,防止公开文案重新出现固定未来版本目标、外部产品名称/对比话术、开发者-only 定位、未验证的中转站/Office 版式过度宣称,以及旧的菱形占位 logo。最新通过报告见 `test-results/product-positioning-audit/latest.json`;v0.1.4 release doctor 已在 release commit `a14c623` 上绑定版本、当前 commit 与干净工作树并达到 `status: ready`。
