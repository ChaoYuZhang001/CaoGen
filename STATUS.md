...
>
> ⚠️ **未达完整发布标准**。`main@e8f617f822be458065dabe7f2440d1f5a33ee2b3` 的 Intel-only 最终候选已通过精确提交四态 Deep、Developer ID 签名、公证、staple、Gatekeeper、DMG 隔离安装、真实 renderer 启动、五资产更新元数据绑定和 clean 后继提交 scoped final-notes audit；Apple Silicon 与 Windows 按创始人决定暂停，不能写成已完成，完整三平台 Release Doctor 因此仍为 `not_ready`。当前未创建 tag/GitHub Release。Docker 已从产品模式删除；正式运行时只保留 OpenAI-compatible 与原生 Anthropic Messages，不需要 Claude Code 登录、SDK 或 CLI。
...
- 正式运行时只有 OpenAI-compatible（Responses / Chat Completions）与原生 Anthropic Messages（`/v1/messages`）两条执行路径，均已注册到 Provider/SessionManager/UI。Claude Code Agent Runtime、Claude Agent SDK 与 Claude CLI 已从依赖、主进程、IPC/UI、测试宿主和打包规则删除；旧 `engine: claude` Provider/会话只迁移为 `anthropic`，不恢复 SDK 隐藏上下文。最终 Intel 候选 Deep 报告 `2026-07-25T08-23-47-652Z` 为 `157 total / 155 required pass / 2 optional skip / 0 blocked / 0 fail`，开始和结束均绑定 clean `e8f617f8`；它只证明当前 Intel 候选和仓库 required gates，不替代暂停平台、真实 Provider、N1 或完整 1.0 验收证据。
...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 Intel x64 v0.1.7 楔子版。** 删除 Claude Runtime 后的最终精确提交 Intel 签名/公证/安装与五资产绑定主链已在 `30150795350` 闭环并独立核对真实签名体积和 SHA256；Release Notes scoped final gate 也已在 clean 后继提交上通过。M1-T6 当前只剩获得明确授权后的 Tag/Release、公开资产审计和官网同步。Apple Silicon 与 Windows 按创始人决定暂停，不计入当前 M1 阻塞项，也不得写成完成。当前候选仍未发布，未经明确发布动作不建 tag/Release。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
...
| 最终候选 Deep | High | ✅ `e8f617f8` 的 Intel-only run `30150795350` 已在 clean exact commit 上通过 `155/155` required；2 项 optional skip 未计作 pass |
