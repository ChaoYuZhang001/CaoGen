...
- v0.1.7 tag 后已有用户可见修复进入 `main`，但公开安装包不包含它们；因此按 `docs/PLAN.md` 的 patch 规则启动 `0.1.8` macOS Intel-only 候选。版本与候选合同锁到 `0.1.8` / `macos-x64`，手动证据 workflow 保持 `contents: read`；Apple Silicon/Windows 不恢复，v0.1.7 远端正文与 3 个 Windows 资产不修改。下述 run 已关闭当前产品提交的候选构建验证和独立下载核验；发布前仍需把精确证据绑定进 clean publication-only 后继，运行 scoped notes/handoff preflight，并获得新的明确授权。
...
**P0**
- 用户反馈快修循环(常设)
- v0.1.8 Intel-only 补丁候选：`03c3fee2 / 30243108279` 已完成 clean Deep、签名公证、安装启动、精确五资产及独立下载核验；当前只推进 publication-only scoped notes/handoff preflight，不含 tag/Release 发布权限
...
| v0.1.8 Intel 补丁候选 | High | 候选证据通过：`03c3fee2 / 30243108279` 已覆盖当前产品修复并完成全链与独立资产核验；仍需 clean publication-only scoped notes/handoff preflight 和新的明确发布授权，不得发布 |
