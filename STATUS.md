...
- M1 文档与 x64 签名基线修复已快进到 `main@7035a6c5`；`package.json` 与 lock 根版本均为 `0.1.7`。该 clean commit 上的完整 Deep 为 `156 total / 153 required pass / 3 optional skip / 0 blocked / 0 fail`，报告为 `test-results/caogen-deep/2026-07-23T07-22-12-853Z/deep-test-report.md`；测试开始和结束均为 clean 且 Git 状态不变。最终候选提交仍需重跑 Deep。
...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 v0.1.7 楔子版。** M1-T1/T2/T5、clean Deep 和 0.1.7+ 三平台 packaging fail-closed 门禁已关闭；Intel x64 本地签名基线仍只是非公证负向样本。下一顺序是在最终 clean commit 注入公证凭据后重建 x64，使包内 provenance、notarize/staple、Gatekeeper、artifact digest、原生安装/启动与 required audit 同时通过；再在 Apple Silicon 与 Windows x64 机器补齐对应平台签名、安装和启动证据，最后以完整 12 资产集完成 packaging、Doctor 与 Release Notes final gate。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
