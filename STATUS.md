...
- M1-T6 已新增只读、fail-closed 的 Intel 发布交接预检：它绑定候选提交、四类报告、五项本地资产、Release Notes、clean 后继提交、远端 `main`，并要求本地/远端 tag 与 GitHub Release 均不存在；全部通过后才输出需创始人明确授权的手工发布命令，脚本自身不创建 tag、Release 或上传资产。发布后公开资产审计现要求每个 GitHub 资产的名称、大小与平台返回的 SHA-256 digest 精确匹配本地候选文件，并拒绝额外本地文件。定向回归、required coding standards、typecheck、build、发布合同、macOS x64 证据门禁、产品定位和 worktree secret scan 已通过。`d5f0a86f` 的 Intel-only run `30157730258` 已通过候选身份与全部源码门禁，但 macOS runner 在 `npm ci` 期间被 GitHub Actions internal error 取消；后续同 SHA 重跑与 run `30157949745` 均在 0 jobs、无仓库命令/日志时被平台判为 workflow failure。它们不构成代码、Deep、签名或打包失败证据；新 docs-only exact commit 将使用新的 concurrency key 重试 Intel 候选。
...
| 新最终候选 Deep | High | ⏳ `e8f617f8` 的 run `30150795350` 只保留为历史证据；`d5f0a86f` 的 run `30157730258` 已过源码门禁但被 GitHub runner internal error 取消，后续同 key 为 0-job 平台失败。必须在新 exact commit 上重跑，2 项 optional skip 仍不得计作 pass |
