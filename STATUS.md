...
> 更新:2026-07-22· 实测口径,非文档自评。此文件为活文档,Current Focus 随日更新。
>
...
> 当前 `1.0.0` 仅是本地候选版本，不是正式 1.0 stable：PRD 64 个 P0 中 21 个处于完整“当前已验证”，43 个仍开放，其中 25 个为立项目标、17 个部分完成、1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；新版 Release Doctor 会把产品验收、正式 macOS 签名/公证和同轮 secret-history 证据作为硬门禁。
>
...
- 正式运行时现有默认 OpenAI-compatible API、可选 Claude Agent SDK，以及已注册到 Provider/SessionManager/UI 的原生 Anthropic Messages 三条执行路径。最新完整四态门禁为 `150 total / 147 required pass / 3 optional skip / 0 blocked / 0 fail`；最新报告为 `test-results/caogen-deep/2026-07-22T03-13-05-163Z/deep-test-report.md`，滚动入口为 `test-results/caogen-deep/latest.md`。3 个 optional skip 分别保留真实 Claude、China real-network、China tool-call parity 的外部条件，不计入 pass，也不阻塞默认发布档位；该报告绑定 `21051cab`，测试开始和结束均为 dirty worktree（456 expanded status entries）且 Git 状态保持不变，不能替代 clean release candidate gate。
...
- P2 本地 smoke 已刷新全绿;P2-005 IDE integrations 已由 `test:p2-ide-build-and-vscode:required`、`test:jetbrains-recorder-e2e:required`、`test:jetbrains-ide-interaction:required` 证明。最新 `npm run test:p2-audit` 报告中 P2-002/P2-003/P2-005 为 proved;`npm run test:p2-audit -- --required` 仍会失败,但失败范围只剩非本轮公开宣称的 P2-001 Windows GUI required evidence 与 P2-004 China external evidence。最新 Doctor 为 `test-results/workos-release-doctor/2026-07-22T09-33-11-558Z`，overall 仍为 `not_ready`：acceptance map 结构、CycloneDX 1.5 SBOM（856 组件，High=0/Critical=0）、packaging、positioning、GitHub release audit 和 secret-history 均有本地证据，但正式 P0 验收、clean candidate/SBOM/Deep 绑定、真实默认 Provider release record、N1 和 macOS distribution 仍开放。七天 soak 未执行，已由 release owner 在 `docs/1.0-SOAK-WAIVER.json` 对精确版本 `1.0.0` 显式接受风险，Doctor 记录为 `waived/non-blocking`，不得记为 pass 或继承到后续版本；对应 audit 为 `test-results/product-1.0-soak-audit/2026-07-22T09-29-58-865Z/report.json`。
...
- Developer ID、release entitlements、Hardened Runtime、API-key notarization 配置均已通过 preflight，且 `notarytool` 已实际向 Apple 认证成功；最新报告 `test-results/macos-release-preflight/2026-07-22T09-50-24-743Z/report.json` 的唯一失败项是 dirty worktree。材料不再是阻塞，剩余工作是对最终 clean candidate 签名、公证、staple 并审计，旧 dirty artifact 不补签。
...
**1.0 当前聚焦执行闭环，Canonical Goal/WorkItem 生产写源、Anthropic 原生生产路径、Acceptance repair/retest、Evidence integrity、TEAM-002 原生招聘真实 Electron 闭环与 AUTO-003/AUTO-004 统一 Learning 生命周期已完成 targeted 收口**：生产 read/write 已 Ledger-first 且 crash-safe，DigitalWorker 招聘/策略、Anthropic Engine/Session/UI、工具循环、保守 failover、图片重启恢复、repair/retest、两类 failure producer、逐 criterion kind/source policy、repair-derived Acceptance policy propagation、Evidence event/source binding、Artifact 真实字节一致性，以及 Memory/Skill draft、可信审批、版本/撤销/回滚/过期/删除、重启审计、fail-closed 物化和有效 Memory prompt 注入均有 required smoke。新 Run 首次投影会冻结所属 Acceptance ID/revision；测试失败和模型仲裁失败均携带该绑定，旧事件首次晚到新 revision 时会在零 Evidence/零 Acceptance mutation 下 fail-closed。最新完整 Deep 仍为 `150 total / 147 required pass / 3 optional skip / 0 blocked / 0 fail`；该报告开始和结束均是 456 expanded status entries 的 dirty-worktree 证据。WORK-003 新增 canonical smoke 已证明两个独立 Session Run 共享一个 WorkItem、启动重放幂等且跨 WorkItem Run 复用 fail-closed，但完整 renderer/SessionManager ingress 仍待真实 E2E。Supervisor 已新增持久状态/IPC、TaskRun→WorkItem identity bridge 与受控 SessionManager pause/cancel/resume/retry/reassign 切片；Studio UI、预算/并发、自动编排、真实 Provider parity 与跨域强杀恢复仍开放，故 RUN-004/WORK-004/NFR-REC-004 仅部分完成、RUN-005 仍开放。下一步按依赖顺序推进其他测试执行路径与自动编排、完整 WorkItem/Supervisor controls、独立 Verification/不可变交付链、统一 Run/Context 契约、TEAM-007 Worker memory namespace 与其余 canonical 业务入口；policy authoring/review UI 也有真实 Electron required 证据。在这些边界关闭前不得宣称 WORK-003、WORK-004、ART-004、RUN-003、ART-002、TRUST-005、TEAM-005、TEAM-007 或 1.0 stable 完成。
...
1. `1.0.0` 不等待连续 7 天日常使用；由 `docs/1.0-SOAK-WAIVER.json` 显式记录 owner、原因、风险和补偿门禁，且只豁免该版本的 elapsed soak，不豁免其他发布门禁
2. 历史 arm64 资产曾发布；1.0 arm64 仍需从最终源码原生构建并在 Apple Silicon 上完成安装、启动、功能和升级验证
3. 默认 OpenAI-compatible 路径无需 Claude 登录；Claude 仅作为用户显式选择的可选专项验证
4. N1 秒表实测 ≤30 分钟(真人,录屏留证)——formal 1.0 stable 硬门禁;未验证前不得发布或宣称正式 1.0 stable
5. `test:deep` 保持至少当前 `150 total / 147 required pass / 3 optional skip / 0 blocked / 0 fail`;required 不得以 skip/blocked 通过
...
| 阻碍 | 等什么 |
|---|---|
| Apple Developer / 签名材料 | Developer ID 和 App Store Connect API-key 已配置并实际认证成功；等待最终 clean candidate 后执行签名、公证、staple 与审计 |
...
2. **分发摩擦**:Developer ID 与 Apple 公证认证前置已通过，但当前 1.0 本地候选仍是未签名 dirty x64 资产；正式 macOS stable 仍缺最终 clean artifact 的签名、公证、stapling 和 Apple Silicon 证据
