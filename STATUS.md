...
- **[v0.1.5 macOS x64 已发布](https://github.com/ChaoYuZhang001/CaoGen/releases/tag/v0.1.5)**(2026-07-14)——五个公开资产已上传并通过名称、大小、状态、文本元数据与 SHA256 审计;tag 精确指向 `d9969e3`,Intel 主二进制为 `x86_64`,arm64/Windows/Linux 不在本轮发布范围
...
- GitHub Releases 公开资产审计已补齐:v0.1.5 发布后已通过 `npm run test:github-release-audit:read-text:required -- --tag v0.1.5 --expected-assets-from-dist`,线上 5 个资产的名称、大小、状态与 SHA256 均和本地 `dist` 一致,`latest-mac.yml` 也已读取;通过报告为 `test-results/github-release-audit/2026-07-14T03-04-30-819Z/report.json`。
