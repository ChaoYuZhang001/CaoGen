...
>
> ⚠️ **尚未正式发布**。`main@cfce93725972167aad0811d381c710e7cd510b26` 的 Intel-only 候选 run `30160236851` 已通过精确提交 Deep、Developer ID 签名、公证、staple、Gatekeeper、DMG 隔离安装、真实 renderer 启动和五资产更新元数据绑定；当前正在把下载的候选证据与最终 Release Notes 绑定到 clean publication-only 后继提交，发布交接预检仍待执行。Apple Silicon 与 Windows 按创始人决定暂停，不能写成已完成，完整三平台 Release Doctor 因此仍为 `not_ready`。当前未创建 tag/GitHub Release，创建它们仍需明确授权。Docker 已从产品模式删除；正式运行时只保留 OpenAI-compatible 与原生 Anthropic Messages，不需要 Claude Code 登录、SDK 或 CLI。
