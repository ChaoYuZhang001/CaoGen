...
- M1-T4 的公开进度口径已对齐：中英文 README、STATUS 与官网统一为 `64 = 21 已验证 + 17 部分完成 + 25 立项目标 + 1 仅达到基础`；官网配置同时强制四类总和等于 P0 总数，源码已推送至 `CaoGen-Website@c2f655d`，`caogen.dev` 已返回包含四分类的新生产构建。该文档闭环不改变 Release Doctor 的 `not_ready` 状态，也不替代签名、公证或目标平台证据。
