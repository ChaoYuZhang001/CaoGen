...
> `0.1.8` Intel-only 未发布候选已刷新到 `main@9591c20bde2330a6f57951e7381b5e7e9d642091`。只读 run [`30215660873`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30215660873) 已通过 clean Deep、签名、公证、安装、renderer 与五资产绑定，候选资产也已独立下载并逐项核验；旧 `837f8f90 / 30212121353` 只保留为历史证据。尚未完成 Final Notes、publication-only 预检或 v0.1.8 发布授权，也未创建 tag/Release。v0.1.7 仍是最新公开版，也不是正式 1.0 stable：PRD 64 个 P0 = 21 个已验证 + 18 个部分完成 + 24 个立项目标 + 1 个仅达到基础。执行边界见 `docs/1.0-ACCEPTANCE-MATRIX.md`；0.1.8 只允许交付已验证补丁，不把路线图能力写成已完成。
>
...
- v0.1.7 tag 后已有用户可见修复进入 `main`，但公开安装包不包含它们；因此按 `docs/PLAN.md` 的 patch 规则启动 `0.1.8` macOS Intel-only 候选。版本与候选合同锁到 `0.1.8` / `macos-x64`，手动证据 workflow 保持 `contents: read`；Apple Silicon/Windows 不恢复，v0.1.7 远端正文与 3 个 Windows 资产不修改。下述 run 已关闭候选构建验证和独立下载核验，发布前仍需 Final Notes、publication-only 预检与新的明确授权。
- `main@9591c20bde2330a6f57951e7381b5e7e9d642091` 的 0.1.8 Intel-only 候选 run [`30215660873`](https://github.com/ChaoYuZhang001/CaoGen/actions/runs/30215660873) 已成功：source gates 与 P2 通过，exact-commit Deep 为 `163 total / 161 required pass / 2 optional skip / 0 blocked / 0 fail`，macOS audit 为 `120/120`，Developer ID 签名、公证/staple、Gatekeeper、DMG 隔离安装/干净 detach、真实 renderer 和 packaged-app smoke 全部通过。五资产集 digest 为 `b5e03719796ea3236fab617c8e1493a238e3a07e48552daa1dc74b04f7d27252`；Apple Silicon、Windows 和完整矩阵汇总均按范围跳过。Actions artifact `8636056669` / `caogen-release-macos-x64-9591c20bde2330a6f57951e7381b5e7e9d642091` 保存候选资产与报告，到期日为 2026-08-09。五项资产已下载到仓库外临时目录并独立复算大小、SHA-256 与资产集 digest，结果和 audit/packaged-app 报告一致。该结果只证明未发布 Intel 候选，不授权 tag、Release、v0.1.7 远端修改，也不关闭真人 M1 或 formal 1.0 门禁。
