...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 v0.1.7 楔子版。** M1-T1/T2/T4/T5、clean Deep、0.1.7+ 三平台 packaging fail-closed 门禁和手动候选 workflow 已关闭；Intel x64 本地签名基线仍只是非公证负向样本。下一顺序是把合并后的 clean `main` SHA 和 0.1.7 交给该 workflow，在仓库 secrets 完整时并行取得 Intel、Apple Silicon、Windows 三条真实证据；再完成 Release Notes final、required Doctor 与公开资产审计。workflow 失败或 Doctor `not_ready` 时不建 tag/Release。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
