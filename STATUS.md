...
- PR #7 已于 2026-07-23 合并到 `main@20fab616`；`package.json` 与 lock 根版本均为 `0.1.7`，合并后工作树为 clean。最新 Release Doctor（`test-results/workos-release-doctor/2026-07-23T05-06-37-849Z`）已将 release identity 标为 ready；当前开放域仅为 final-commit Deep、0.1.7 packaging/signing 和 final Release Notes。
- 正式运行时现有默认 OpenAI-compatible API、可选 Claude Agent SDK，以及已注册到 Provider/SessionManager/UI 的原生 Anthropic Messages 三条执行路径。最新完整四态门禁为 `156 total / 153 required pass / 3 optional skip / 0 blocked / 0 fail`；最新报告为 `test-results/caogen-deep/2026-07-23T04-14-55-598Z/deep-test-report.md`，滚动入口为 `test-results/caogen-deep/latest.md`。3 个 optional skip 分别保留真实 Claude、China real-network、China tool-call parity 的外部条件，不计入 pass，也不阻塞默认发布档位；该报告绑定 `21051cab`，测试开始和结束均为 dirty worktree（547 expanded status entries）且 Git 状态保持不变，不能替代 clean release candidate gate。
...
- P2 本地 smoke 已刷新全绿;P2-005 IDE integrations 已由 `test:p2-ide-build-and-vscode:required`、`test:jetbrains-recorder-e2e:required`、`test:jetbrains-ide-interaction:required` 证明。最新 `npm run test:p2-audit` 报告中 P2-002/P2-003/P2-005 为 proved;`npm run test:p2-audit -- --required` 仍会失败,但失败范围只剩非本轮公开宣称的 P2-001 Windows GUI required evidence 与 P2-004 China external evidence。最新 Doctor 为 `test-results/workos-release-doctor/2026-07-23T05-06-37-849Z`，overall 仍为 `not_ready`：0.1.7 release identity 已 ready，0.1.x 不要求正式 1.0 P0 closure、真实 Provider record、N1 或七天 soak；当前阻塞是最终提交上的 clean Deep、0.1.7 packaging/signing 与 final Release Notes。
...
**当前唯一焦点是 M1：发布已签名、可安装、诚实标注边界的 v0.1.7 楔子版。** PR #7 已关闭 M1-T1/T2；本轮收敛旧计划和 README，关闭 M1-T5。下一顺序是最终文档提交上的 clean Deep，然后生成 0.1.7 macOS x64/arm64 与 Windows x64 候选资产，完成签名、公证、staple、安装启动审计和 Release Notes final gate。M1-M3 期间冻结新的 1.0 愿景功能；64 个 P0 仍为 21 已验证、43 开放，不因发布 0.1.7 而改变。
