...
const MULLION_COUNT = Math.floor(WALL_W / MULLION_GAP) + 1
...
export default function WindowWall({
  ...
  const skyGeo = useMemo(() => {
    const g = new PlaneGeometry(WALL_W + 8, WALL_H + 6, 1, 48)
    const pos = g.attributes.position
    const colors = new Float32Array(pos.count * 3)
    const c = new Color()
    const half = (WALL_H + 6) / 2
    for (let i = 0; i < pos.count; i++) {
      const y = pos.getY(i)
      const t = (y + half) / (WALL_H + 6)
      c.copy(skyBottom).lerp(skyTop, t)
      const horizon = Math.max(0, 1 - Math.abs(t - 0.1) / 0.14)
      c.lerp(horizonGlow, horizon * 0.35)
      colors[i * 3] = c.r
      colors[i * 3 + 1] = c.g
      colors[i * 3 + 2] = c.b
    }
  ...
  useLayoutEffect(() => {
    ...
    for (let i = 0; i < MULLION_COUNT; i++) {
      const x = -WALL_W / 2 + i * MULLION_GAP
  ...
  useLayoutEffect(() => {
    ...
    cityData.buildings.forEach((b, i) => {
      dummy.position.set(b.x, b.y, BACKDROP_Z + 0.6 + hash(i * 7.9) * 2.2)
      dummy.rotation.set(0, 0, 0)
  ...
  return (
    <group ref={groupRef} position={position} rotation={rotation} scale={scale}>
      {/* 窗外天空渐变背板(发光,供 Bloom 轻微辉光) */}
      <mesh geometry={skyGeo} position={[0, WALL_H / 2, BACKDROP_Z]}>
      ...
      {/* 城市发光窗点(供 Bloom) */}
      <instancedMesh ref={windowsRef} args={[undefined, undefined, WINDOW_COUNT]}>
      ...
      <instancedMesh
        ref={mullionsRef}
        args={[undefined, undefined, MULLION_COUNT]}
      >
      ...
      {/* 底部青色氛围灯带(点缀,发光) */}
      <mesh position={[0, 0.14, GLASS_Z + 0.02]}>
        <boxGeometry args={[WALL_W, 0.03, 0.02]} />
        <meshStandardMaterial
          color="#8fe9ff"
          emissive="#8fe9ff"
