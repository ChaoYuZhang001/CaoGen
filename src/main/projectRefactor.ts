...
type JournalFileState = 'before' | 'after' | 'drift' | 'unavailable'
...
async function reconcileJournal(record: ProjectRefactorJournalRecord): Promise<ProjectRefactorJournalRecord> {
  ...
  const root = await normalizeJournalRoot(record)
  if (!root) return record.stage === 'applied' ? record : blockJournal(record, 'Workspace is unavailable')
  if (!samePath(root, record.root)) return blockJournal(record, 'Workspace identity changed')
  const states = await inspectJournalSnapshots(root, record.snapshots)
  return record.stage === 'applied'
    ? reconcileAppliedJournal(record, states)
    : reconcileInterruptedJournal(record, root, states)
}

async function normalizeJournalRoot(record: ProjectRefactorJournalRecord): Promise<string | null> {
  try {
    return await normalizeRoot(record.root)
  } catch {
    return null
  }
}

async function inspectJournalSnapshots(root: string, snapshots: ProjectRefactorJournalSnapshot[]): Promise<JournalFileState[]> {
  const states: JournalFileState[] = []
...
async function reconcileAppliedJournal(record: ProjectRefactorJournalRecord, states: JournalFileState[]): Promise<ProjectRefactorJournalRecord> {
  if (states.includes('unavailable')) return record
  return states.every((state) => state === 'after') ? record : supersedeJournal(record)
}

async function reconcileInterruptedJournal(
  record: ProjectRefactorJournalRecord,
  root: string,
  states: JournalFileState[]
): Promise<ProjectRefactorJournalRecord> {
  ...
  try {
    await restoreInterruptedSnapshots(root, record.snapshots, states)
...
async function restoreInterruptedSnapshots(
  root: string,
  ...
  for (let index = 0; index < snapshots.length; index += 1) {
    ...
    await transitionSnapshot(root, snapshots[index], 'after', 'before')
  }
}
