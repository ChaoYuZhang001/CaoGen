...
function validateRecord(value: unknown): asserts value is ProjectRefactorJournalRecord {
  ...
  validateRecordMetadata(value)
  validateRecordSnapshots(value)
  validateRecordTimestamps(value)
}

function validateRecordMetadata(value: Record<string, unknown>): void {
  if (value.version !== JOURNAL_VERSION || value.kind !== 'typescript-rename') throw new Error('Project refactor journal record is invalid')
  if (!OPERATION_ID.test(String(value.operationId))) throw new Error('Project refactor journal operation ID is invalid')
  if (typeof value.sessionId !== 'string' || !value.sessionId || value.sessionId.length > 512) throw new Error('Project refactor journal session ID is invalid')
  if (typeof value.root !== 'string' || !path.isAbsolute(value.root) || value.root.includes('\0')) throw new Error('Project refactor journal root is invalid')
  if (!['applying', 'applied', 'rolling_back', 'recovered', 'superseded', 'blocked'].includes(String(value.stage))) {
    throw new Error('Project refactor journal stage is invalid')
  }
  if (!SHA256.test(String(value.integrity))) throw new Error('Project refactor journal integrity is invalid')
}

function validateRecordSnapshots(value: Record<string, unknown>): void {
  if (!Array.isArray(value.snapshots) || value.snapshots.length === 0 || value.snapshots.length > 200) {
    throw new Error('Project refactor journal snapshots are invalid')
  }
  for (const snapshot of value.snapshots) validateSnapshot(snapshot)
}

function validateRecordTimestamps(value: Record<string, unknown>): void {
  if (!validTimestamp(value.createdAt) || !validTimestamp(value.updatedAt)) throw new Error('Project refactor journal timestamps are invalid')
