...
> 2026-07-04 · 终极产品目标:**让其他所有 Agent 的深度用户丝滑转用 CaoGen;CaoGen 就是世界第一,中国首创。** 第一纪元(M0–M7)证明了"拥有"三支柱;第二纪元(M8–M15)要求**好用**——任何主流 Agent 的深度用户迁移后第一天不丢肌肉记忆、第一周工作流不倒退、第一个月离不开。

判断标准不是口号,而是迁移矩阵、迁移漏斗、工程隔离、真子代理编排、桌面工作台、记忆自动化和插件生态的逐项验收。

## 0. 设计总纲

### 0.0 迁移矩阵:"所有 Agent"的深度依赖与 CaoGen 对应

"所有"意味着逐个拆解每类 Agent 深度用户的依赖资产与肌肉记忆,给出对应与导入物。这是 D11 迁移向导的需求清单:

| 来源 Agent | 深度用户依赖什么 | CaoGen 对应 | 可自动导入的资产 |
|---|---|---|---|
| **Claude Code** | `@`/`/`/Esc Esc、CLAUDE.md、`.claude/`(skills/agents/hooks)、MCP | 同源引擎,原生 1:1(D1/D2/D8) | 全部零改动继承(已通) |
| **Codex CLI** | AGENTS.md、`~/.codex/config.toml`、审批模式、cloud tasks | D1 交互 + 权限四模式 + D7 Routines | AGENTS.md→CLAUDE.md 引用注入;MCP 配置翻译 |
| **Gemini CLI** | GEMINI.md、内置 Google 搜索、免费额度习惯 | M6 GeminiEngine(架构已留位)+ 搜索 MCP | GEMINI.md→CLAUDE.md 引用注入 |
| **Cursor** | `.cursor/rules/*.mdc`、Tab 补全、Composer 多文件、内嵌编辑器 | rules 导入 + D4 编辑器/Diff(不做 Tab 补全——非 Agent 品类,明确不追) | `.cursor/rules`、`.cursorrules`→规则注入;MCP json |
| **Windsurf** | `.windsurf/rules`、Cascade 流、预览 | rules 导入 + D4 预览/浏览器 | rules 文件注入 |
| **Cline / Roo** | `.clinerules`、MCP marketplace、Plan/Act 模式 | 规则导入 + plan 权限模式(已有)+ D9 插件浏览器 | `.clinerules`、MCP settings json |
| **Aider** | git 原生工作流(自动 commit)、`--watch`、repo map | D3 worktree + git 语义合并;repo map≈@补全+CLAUDE.md | `.aider.conf.yml` 关键项(model→Provider 映射) |
| **GitHub Copilot** | IDE 内嵌、PR 审查、`.github/copilot-instructions.md` | D4 工作台 + D7 GitHub 桥接 | copilot-instructions.md→规则注入 |
| **通义灵码 / Trae 等国产** | 中文交互、国产模型、企业内网 | 中文原生 UI + 国产模型直连预设 + 网关兼容 | 规则文件(通用格式)注入 |
| **OpenHands / Devin 类** | 全自主长任务、浏览器操作 | D5 子代理 + D6 浏览器 + D7 Routines | (会话资产不可移植,靠能力对齐) |

**迁移矩阵三原则**:
1. **资产不丢**:规则/记忆/MCP/技能都有去处,导入时标注来源、先备份、可回滚;
2. **习惯不改**:高频动作(@、/、图片、回退、审批)1:1;来源 Agent 特有强项(如 Cursor Tab 补全)若属别的品类,明确宣告"不做"而非做残;
3. **信任升级**:迁移后获得原工具没有的安全网(worktree + checkpoint + 透明成本),这是留下来的理由。
...
## D11 · 迁移向导:一键搬家(M8 起步,随纪元完善)

**体验目标**:Cursor 用户第一次打开 CaoGen,它说:"检测到你在用 Cursor,发现 3 个 rules 文件、2 个 MCP server。一键导入?"点一下,新会话里 agent 已经懂这个仓库的规矩。**迁移的第一分钟决定去留。**

**方案**:
- **检测器**(主进程 `migration.ts`):扫描两级——用户级(`~/.claude`、`~/.codex`、`~/.cursor`、`~/.aider.conf.yml` 等)与项目级(cwd 下 `.cursor/rules`、`.cursorrules`、`.windsurf/rules`、`.clinerules`、`AGENTS.md`、`GEMINI.md`、`.github/copilot-instructions.md`);产出 `MigrationScan {agent, assets: {kind, path, preview}[]}`。
- **导入策略**(按资产类型):
  - **规则/记忆文件** → 注入项目 `CLAUDE.md` 的 `<!-- caogen:imported from X -->` 区块(保留原文 + 来源标注;已有内容先备份 `CLAUDE.md.bak`);引擎经 systemPrompt preset 自动生效;
  - **MCP 配置** → 翻译为 `.mcp.json`(Claude 格式)合并,冲突(同名 server)跳过并提示;
  - **模型偏好**(如 aider conf 的 model)→ 建议映射到已配 Provider,一键采纳;
  - 不可自动迁移的(如 Cursor Tab 习惯)→ 显示"CaoGen 对应做法"说明卡。
- **入口**:设置页"迁移"标签 + 新建会话时检测到项目级资产弹提示条(可永久关闭)。
- **红线**:只读扫描,导入前逐项勾选确认;绝不上传;绝不改动来源工具的文件。

**验收**:在有 `.cursorrules` + `.mcp.json`(Cline 格式)的仓库运行向导,导入后新会话 agent 遵守规则且 MCP 工具可用;`CLAUDE.md.bak` 存在;来源文件未被改动。
